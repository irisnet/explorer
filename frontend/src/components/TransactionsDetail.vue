    ...
    mounted() {
      ...
      axios.get(url).then((data)=>{
        ...
        if(data && typeof data === "object"){
          ...
          this.gasPrice = Tools.scientificNotationConversionNumber(Tools.formatNumber(data.GasPrice));
          ...
          if(data.Amount && data.Amount.length !==0){
            this.amountValue = data.Amount.map(item=>{
              item.amount = Tools.scientificNotationConversionNumber(Tools.formatNumber(item.amount));
            ...
            data.Amount =  Tools.scientificNotationConversionNumber(Tools.formatNumber(data.Amount.amount));
            this.amountValue = `${data.Amount.amount} ${Tools.formatDenom(data.Amount.denom).toUpperCase()}`
          }else {
            this.amountValue = "--"
          }
          this.actualTxFee = `${Tools.scientificNotationConversionNumber(Tools.formatNumber(data.Fee.amount))} ${Tools.formatDenom(data.Fee.denom).toUpperCase()}`;
          ...
          if(data.Type === "Transfer" || data.Type === "Delegate" || data.Type === "BeginUnbonding" || data.Type === "CompleteUnbonding"){
            ...
            if(data.SelfBond && data.SelfBond.length !== 0){
              this.selfBond = `${Tools.scientificNotationConversionNumber(Tools.formatNumber(data.SelfBond[0].amount))} ${Tools.formatDenom(data.SelfBond[0].denom).toUpperCase()}`;
            }else {
              this.selfBond = "--"
            }
