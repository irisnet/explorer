<template>
  <div class="blocks_list_page_wrap">
    <div class="blocks_list_title_wrap">
      <p :class="stakeListPageWrap">
        <span class="blocks_list_title">{{listTitleName}}</span>
      </p>
    </div>
    <div :class="stakeListPageWrap">
  ...
  export default {
    ...
    data() {
      return {
        ...
        stakeListPageWrap: 'personal_computer_blocks_list_page',
        currentPage: Number(this.$route.query.pagenum),
        pageSize: 30,
        count: localStorage.getItem("stakeListCount") ? Number(localStorage.getItem("stakeListCount")) : 0,
        items: [],
        type: 'list',
        showNoData: false,
        showLoading: false,
    ...
    beforeMount() {
      this.type = this.$route.params.type;
      this.stakeListPageWrap = Tools.getClassByWindowInnerWidth(window.innerWidth);
    },
    mounted() {
      this.getDataList(Number(this.$route.query.pagenum), this.pageSize);
      this.listTitleName = 'Stake';
    ...
    methods: {
      getDataList(currentPage, pageSize) {
        ...
        let url = `/api/tx/stake/${currentPage}/${pageSize}`;
        let that = this;
        axios.get(url).then((data) => {
          ...
          if(data.code == 0){
            ...
            localStorage.setItem("stakeListCount",this.count);
            if(data.data.Data){
            ...
            localStorage.setItem("stakeListCount",0);
          }
        })
          .catch(e => {
            localStorage.setItem("stakeListCount",0);
