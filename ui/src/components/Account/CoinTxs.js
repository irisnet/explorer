...
class CoinTxs extends Component {
    ...
    render() {
        ...
        <Table.HeaderCell width={5}>To</Table.HeaderCell>
            ...
            <Table.Body>
            {txs.map((tx, index) => <CoinTx key={index} height={tx.height} tx={tx} />)}
        </Table.Body>
        </Table>
    ...
    getCoinLabel(tx, address) {
        let isFrom = tx.from == address
        let isTo = tx.to == address
    ...
    render() {
        ...
        const label = this.getCoinLabel(this.props.tx, address)
        ...
        return (
            ...
            <Table.Cell>
            <InOut key={1} address={tx.from} coins={tx.amount} />
        </Table.Cell>
        <Table.Cell>
        <CoinLabel label={label} />
        </Table.Cell>
        <Table.Cell>
        <InOut key={1} address={tx.to} coins={tx.amount} />
        </Table.Cell>
        <Table.Cell>
